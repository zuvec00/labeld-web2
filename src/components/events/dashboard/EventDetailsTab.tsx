...
export default function EventDetailsTab({ eventId }: { eventId: string }) {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				if (!eventData) {
					// Handle not found if needed, or pass error up
					// router.push("/events");
	...
	}, [eventIdString]); // Removed router from deps
	...
	async function saveChanges() {
		...
		try {
			...
			// We removed router.push since we are already on the page
		} catch (e: any) {
	...
	// If err but no event, show error state
	if (err && !event) {
		return (
	...
	return (
		<>
			<ErrorModal
				...
				onClose={() => setErr(null)}
			/>
			<div className="py-6 max-w-2xl mx-auto pb-24">
				{/* No Header needed as main page has header */}
				...
				{/* Actions */}
				<div className="fixed inset-x-0 bottom-0 bg-bg/80 backdrop-blur border-t border-stroke px-3 sm:px-4 lg:px-6 py-3 sm:py-4 flex items-center justify-end max-w-2xl mx-auto z-10">
					{/* <Button
						text="Cancel"
						variant="outline"
