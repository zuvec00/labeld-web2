		...
		<div className="space-y-3 pb-20 md:pb-0">
			{pieces.map((piece) => {
				...
				return (
					<div
						key={piece.id}
						className="relative flex items-center gap-4 p-3 rounded-xl border border-stroke bg-bg hover:bg-surface/50 transition-colors cursor-pointer"
						onClick={() => onOpen(piece)}
					>
						{/* Leading: Image */}
						<div className="w-12 h-16 flex-shrink-0 bg-surface rounded-md overflow-hidden border border-stroke/50">
							<img
						...
						<div className="flex-1 min-w-0 flex flex-col justify-center gap-0.5">
							<h3 className="text-xs md:text-sm font-light text-text truncate pr-2">
								{piece.dropName}
							</h3>
						...
						{/* Trailing: Price & Menu */}
						<div className="flex flex-col items-end justify-between h-14 gap-1">
							...
							<button
								...
								className="p-1.5 -mr-2 text-text-muted hover:text-text hover:bg-surface-neutral/50 rounded-lg transition-colors"
							>
						...
						{openMenuId === piece.id && (
							<>
								...
								/>
								<div className="absolute right-2 top-12 z-30 w-32 bg-surface border border-stroke rounded-xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
