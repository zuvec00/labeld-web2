 ...
 *
 * This page initializes the isOpen field on ALL existing brands
 * across the entire database. After running once, DELETE this file.
 *
 * Fields initialized:
 * - isOpen: false (brand store is closed by default)
 * - updatedAt: server timestamp
 */

export default function InitializeBrandsPage() {
	...
	async function initializeBrandFields() {
		const auth = getAuth();
		...
		setStatus("running");
		setProgress("🔍 Fetching all brands...");

		try {
			const db = getFirestore();

			// Fetch ALL brands across entire database
			const brandsSnapshot = await getDocs(collection(db, "brands"));
			const total = brandsSnapshot.size;

			setProgress(
				`🏢 Found ${total} brands across the database. Initializing...`
			);
			...
			for (const docSnap of brandsSnapshot.docs) {
				try {
					...
					// Initialize isOpen to false if not set
					...
					const msg = `Error on brand ${docSnap.id}: ${err}`;
					console.error(msg);
					errors.push(msg);
				}
			}
			...
			console.log(`
╔════════════════════════════════════════╗
║   Brands Initialization Complete ✅   ║
╠════════════════════════════════════════╣
║  Total Brands:      ${total.toString().padStart(6)}          ║
			...
			<div className="max-w-3xl mx-auto">
				...
				<div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 mb-6">
					<div className="flex items-center gap-4 mb-4">
						...
						<div>
							...
							<p className="text-gray-500 dark:text-gray-400 mt-1">
								One-time setup for ALL brands (database-wide)
							</p>
						</div>
					</div>
					...
					<div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 mt-6">
						...
						<p className="text-sm text-yellow-700 dark:text-yellow-300 mt-2">
							This will initialize the following field on ALL existing brands
						...
						<ul className="text-sm text-yellow-700 dark:text-yellow-300 mt-2 ml-4 space-y-1">
							<li>
								...
								: false (brand store is closed by default)
							</li>
							<li>
								•{" "}
								<code className="bg-yellow-100 dark:bg-yellow-900/40 px-1 rounded">
					...
					{status === "idle" && (
						<div className="text-center py-8">
							<p className="text-gray-600 dark:text-gray-300 mb-6">
								Ready to initialize the isOpen field for all brands?
							</p>
							<Button
								text="🚀 Initialize Brand Fields"
								variant="primary"
								onClick={initializeBrandFields}
						...
						<div className="py-8">
							...
							<div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
								...
								<ol className="text-sm text-purple-700 dark:text-purple-300 space-y-2 ml-4">
									<li>
										1. Verify your brands have the new isOpen field in Firestore
									</li>
									...
									<li className="font-mono text-xs bg-purple-100 dark:bg-purple-900/40 p-2 rounded mt-1">
										rm src/app/(protected)/(dashboard)/init-brands/page.tsx
									</li>
									<li>3. Brands can now toggle their store open/closed! 🎉</li>
								</ol>
							</div>
						</div>
					)}
				...
				<div className="mt-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
					...
					<ul className="text-sm text-blue-800 dark:text-blue-200 space-y-2">
						<li>• Fetches ALL brands from the entire database</li>
						<li>• Only updates brands that are missing the isOpen field</li>
						<li>
						...
						<li>
							...
							timestamp on modified brands
						</li>
						<li>
							• All brands will have{" "}
							<code className="bg-blue-100 dark:bg-blue-900/40 px-1 rounded">
								isOpen: false
							</code>{" "}
							by default
