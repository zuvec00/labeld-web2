...
type Tab = "free" | "paid"; // "invite" commented out for MVP
...
export default function TicketsStepPage() {
	...
	return (
		<div className="min-h-dvh px-4 sm:px-6 py-8 max-w-3xl mx-auto">
			<Stepper steps={STEPS} activeKey="tickets" eventId={eventIdString} />
...
function CreateSingleDialog({
	...
	// //Commented out for MVP - invite functionality will be implemented later
	// useEffect(() => {
	// 	if (tab === "invite") {
	// 		setAdmitType("vip");
	// 	} else {
		...
		<DialogFrame title="Add a new single ticket" onClose={onClose}>
			...
			<div className="inline-flex rounded-xl bg-surface p-1 border border-stroke">
				{[
					{ key: "free", label: "Free" },
					{ key: "paid", label: "Paid" },
					// { key: "invite", label: "Invite-only" }, // disabled for MVP
				].map((it) => (
					<button
						key={it.key}
						className={[
