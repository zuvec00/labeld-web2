...
export default function CollectionPage() {
	...
	// Redirect to the collections tab in brand-space when complete
	useEffect(() => {
		if (state.status === "complete") {
			router.replace("/brand-space?tab=collections");
		}
	}, [state.status, router]);
			...
			<div className="min-h-dvh grid place-items-center">
				<div className="text-center">
					...
					<p className="text-sm text-text-muted mt-2">
						You need an account to access Collections.
					</p>
					<Button
						text="Go to Sign In"
						className="mt-4"
		...
		return (
			<div className="min-h-dvh grid place-items-center px-6">
				<div className="max-w-md text-center">
					...
					<p className="text-sm text-text-muted mt-2">{copy.subtitle}</p>
					<Button
						text={copy.cta}
						className="mt-5"
						variant="primary"
	...
	return (
		<div className="min-h-dvh grid place-items-center">
			<Spinner size="lg" />
			<p className="text-text-muted">Redirecting to Collections...</p>
		</div>
	);
}
